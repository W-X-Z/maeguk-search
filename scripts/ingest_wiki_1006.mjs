...
const STOPWORDS = new Set([
  ...
  // 명단 문서 안의 소분류·개념어 (인명 아님)
  '을사오적', '정미칠적', '경술국적', '사회단체', '친일단체', '종교단체',
  '언론기관', '문화기관', '관변단체', '부일협력', '창씨개명', '조선귀족',
  '고등경찰', '판검사', '군수공업', '전쟁협력', '학병권유', '황민화',
]);
...
async function main() {
  ...
  // 전수 육안 확인용 — 미리보기든 적재든 항상 저장
  const listFile = 'extracted_names.txt';
  writeFileSync(
    listFile,
    people
      .map(
        (p) =>
          `${p.decision_round ?? '?'}기\t${p.name_ko}${p.name_hanja ? `(${p.name_hanja})` : ''}`,
      )
      .join('\n') + '\n',
  );
  console.log(`전체 추출 목록을 ${listFile} 에 저장했습니다. 이상한 항목이 없는지 훑어보세요.`);
